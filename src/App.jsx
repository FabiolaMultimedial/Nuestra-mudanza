...
import {
  ...
  ClipboardList, Activity as ActivityIcon, MapPin, UserPlus, Pencil, LogOut, Menu
} from "lucide-react";
...
const photoUrl = (key, w = 400, h = 400) => {
  if (typeof key === "string" && key.startsWith("http")) return key; // foto real subida por el usuario
  const seed = imgSeed[key] || key || "furniture";
...
const mapItem = (r) => ({ id: r.id, name: r.name, room: r.room, decision: r.decision, by: r.by_name, img: r.img || "generic", salePrice: r.sale_price, saleStatus: r.sale_status, notes: r.notes || "" });
  ...
  return (
    <div className="pb-4">
      <div className="px-5 pt-6">
        <p className="text-xs font-semibold tracking-wide uppercase mb-2" style={{ color: C.sub }}>Nuestra mudanza</p>
...
const SALE_STATUSES = ["Sacar fotos", "Publicado", "Reservado", "Vendido"];
const saleStatusToneMap = { "Sacar fotos": C.yellow, "Publicado": C.sky, "Reservado": C.lilac, "Vendido": C.mint };

function ItemDetailSheet({ item, onClose, onDecide, onDelete, onUpdateDetails }) {
  const options = [
  ...
  useEffect(() => {
    setPrice(item?.salePrice != null ? String(item.salePrice) : "");
    setNotes(item?.notes || "");
  }, [item?.id]);

  if (!item) return <BottomSheet open={false} onClose={onClose} />;

  const isSelling = item.decision === "Vender";
  ...
  return (
    <BottomSheet open={!!item} onClose={onClose} title={item.name}>
      <div>
        ...
        {isSelling && (
          <div className="mb-4 p-4 rounded-[20px]" style={{ background: C.peach.bg }}>
            <p className="text-sm font-bold mb-3" style={{ color: C.peach.fg }}>Detalles de la venta</p>
            <FieldLabel>Precio</FieldLabel>
            <TextInput
              type="number" inputMode="numeric" placeholder="Ej: 50000"
              value={price}
              onChange={(e) => setPrice(e.target.value)}
              onBlur={() => onUpdateDetails(item.id, { sale_price: price === "" ? null : Number(price) })}
            />
            <div className="mt-3">
              ...
              <div className="flex gap-2 mt-1.5 flex-wrap">
                {SALE_STATUSES.map((s) => (
                  <Pill
                    key={s}
                    tone={saleStatusToneMap[s]}
                    active={item.saleStatus === s}
                    onClick={() => onUpdateDetails(item.id, { sale_status: s })}
                  >
                    {s}
                  </Pill>
                ))}
            ...
            {item.saleStatus === "Vendido" && (
              <p className="text-xs mt-3 font-semibold" style={{ color: C.mint.fg }}>
                ✓ Se sumó automáticamente al Presupuesto.
              </p>
            )}
          </div>
        )}
        ...
        <div className="mb-4">
          ...
          <textarea
            ...
            onBlur={() => onUpdateDetails(item.id, { notes })}
            placeholder="Ej: publicado en Marketplace, coordinar retiro..."
            rows={3}
            className="w-full mt-1 p-3 rounded-[16px] text-sm outline-none resize-none"
            style={{ background: C.line, color: C.ink }}
          />
        </div>
...
function itemsToSaleEntries(items) {
  return items
    .filter((i) => i.decision === "Vender")
    .map((i) => ({
      id: `item-${i.id}`,
      item: i.name,
      room: i.room,
      estimated: null,
      listed: i.salePrice ?? null,
      status: i.saleStatus || "Sacar fotos",
      fromItem: true,
    }));
}
...
function SalesScreen({ sales, items, onBack, onOpenAdd }) {
  ...
  return (
    <div className="pb-4">
      ...
      {combined.length === 0 ? (
        <EmptyState title="Nada para vender todavía." subtitle="Marcá un objeto como 'Vender' en Mis cosas, o registrá una venta acá." cta="+ Registrar venta" onCta={onOpenAdd} />
      ) : (
        <div className="px-5 space-y-3">
          {combined.map((s) => (
            <div key={s.id} className="p-4 rounded-[22px] flex items-center gap-3" style={{ background: C.card }}>
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs" style={{ color: C.sub }}>
                  {s.room}{s.estimated ? ` · est. ${money(s.estimated)}` : ""}{s.listed ? ` · precio ${money(s.listed)}` : ""}
                </p>
              </div>
...
/* PRESUPUESTO                                                            */
/* ---------------------------------------------------------------------- */
function BudgetScreen({ sales, items, onBack }) {
  const gastos = 245000;
  const combined = [...itemsToSaleEntries(items), ...sales];
  const ventas = combined.filter((s) => s.status === "Vendido").reduce((a, s) => a + (s.listed || 0), 0);
...
function AddSheet({ open, onClose, initialStep = "menu", onCreateItem, onCreateTask, onCreateBox, onCreateSale }) {
  ...
  const resetAll = () => {
    setName(""); setRoom("Living"); setPhotoFile(null); setPhotoPreview(null);
  ...
  return (
    <BottomSheet open={open} onClose={close} title={titles[step]}>
      ...
      {step === "item" && (
        <div className="space-y-4">
          {photoPreview && (
            <img src={photoPreview} alt="" className="w-full h-40 object-cover rounded-[20px]" />
          )}
          <div className="flex gap-3">
            <input ref={cameraInputRef} type="file" accept="image/*" capture="environment" className="hidden" onChange={onPickPhoto} />
            <input ref={galleryInputRef} type="file" accept="image/*" className="hidden" onChange={onPickPhoto} />
            <button type="button" onClick={() => cameraInputRef.current?.click()} className="flex-1 flex flex-col items-center justify-center gap-1.5 py-6 rounded-[20px]" style={{ background: C.line }}>
              <Camera size={20} style={{ color: C.ink }} />
              <span className="text-xs font-semibold" style={{ color: C.ink }}>Tomar foto</span>
            </button>
            <button type="button" onClick={() => galleryInputRef.current?.click()} className="flex-1 flex flex-col items-center justify-center gap-1.5 py-6 rounded-[20px]" style={{ background: C.line }}>
          ...
          <button
            ...
            onClick={async () => {
              ...
              await onCreateItem({ name, room, photoFile });
      ...
      {step === "task" && (
        <div className="space-y-4">
          ...
          <button
            ...
            onClick={() => {
              const dueLabel = taskDue
                ? new Date(taskDue + "T00:00:00").toLocaleDateString("es-AR", { day: "numeric", month: "short" })
                : "sin fecha";
              onCreateTask({ title: taskTitle, category: taskCategory, due: dueLabel, priority: taskPriority });
              close();
            }}
            className="w-full py-3.5 rounded-full text-sm font-bold"
...
function TopBar({ onOpenMenu, title = "Nuestra mudanza" }) {
  return (
    <div
      ...
      style={{ background: C.bg, height: 56, borderBottom: `1px solid ${C.line}` }}
    >
...
function SidebarMenu({ open, onClose, tab, setTab }) {
  const items = [
  ...
  return (
    <div className="fixed inset-0 z-40">
      <div className="absolute inset-0" style={{ background: "rgba(23,23,23,0.35)" }} onClick={onClose} />
      <div
        className="absolute left-0 top-0 bottom-0 flex flex-col py-6"
          ...
          {items.map((it) => {
            ...
            return (
              <button
                ...
                className="flex items-center gap-3 px-3 py-3 rounded-[16px] text-left"
                style={{ background: active ? C.line : "transparent" }}
              >
                <it.icon size={19} style={{ color: active ? C.ink : C.sub }} strokeWidth={active ? 2.4 : 2} />
                <span className="text-sm font-semibold" style={{ color: active ? C.ink : C.sub }}>{it.label}</span>
              </button>
            );
          })}
        </div>
...
export default function App() {
  ...
  const logActivity = async (who, action, type) => {
    ...
    setActivity((prev) => [{ id: tempId, who, action, type, time: "ahora" }, ...prev]);
    const { data } = await supabase.from("activity").insert({ household_id: householdId, who, action, type }).select().single();
    if (data) setActivity((prev) => prev.map((a) => (a.id === tempId ? mapActivity(data) : a)));
  };
  ...
  const decide = async (id, decision) => {
    ...
    if (decision === "Vender" && it && !it.saleStatus) patch.sale_status = "Sacar fotos";
    setItems((prev) => prev.map((i) => (i.id === id ? { ...i, decision, ...(patch.sale_status ? { saleStatus: patch.sale_status } : {}) } : i)));
    setSelectedItem((prev) => (prev && prev.id === id ? { ...prev, decision, ...(patch.sale_status ? { saleStatus: patch.sale_status } : {}) } : prev));
  ...
  const updateItemDetails = async (id, patch) => {
    const localPatch = {};
    if ("sale_price" in patch) localPatch.salePrice = patch.sale_price;
    if ("sale_status" in patch) localPatch.saleStatus = patch.sale_status;
    if ("notes" in patch) localPatch.notes = patch.notes;
    setItems((prev) => prev.map((i) => (i.id === id ? { ...i, ...localPatch } : i)));
    setSelectedItem((prev) => (prev && prev.id === id ? { ...prev, ...localPatch } : prev));
    const { error } = await supabase.from("items").update(patch).eq("id", id);
    if (error) { console.error("Error actualizando objeto:", error); alert("No se pudo guardar: " + error.message); return; }
    if (patch.sale_status === "Vendido") {
      const it = items.find((i) => i.id === id);
      if (it) logActivity(meName, `vendió ${it.name}${localPatch.salePrice ? ` por $${localPatch.salePrice.toLocaleString("es-AR")}` : ""}`, "ventas");
    }
  };
  ...
  const addItem = async ({ name, room, photoFile }) => {
    let img = ["sofa", "desk", "lamp", "shelf", "table", "mirror", "generic"][items.length % 7];
    if (photoFile) {
      const path = `${householdId}/${Date.now()}-${photoFile.name}`;
    ...
    const { data, error } = await supabase.from("items").insert({ household_id: householdId, name, room, decision: "Sin decidir", img, by_name: meName }).select().single();
    if (error) { console.error("Error creando objeto:", error); alert("No se pudo guardar el objeto: " + error.message); return; }
    setItems((prev) => [mapItem(data), ...prev]);
    logActivity(meName, `agregó ${name}`, "objetos");
  };

  const addTask = async ({ title, category, due, priority }) => {
    const { data, error } = await supabase.from("tasks").insert({ household_id: householdId, title, category, due, priority, done: false, by_name: meName }).select().single();
    if (error) { console.error("Error creando tarea:", error); alert("No se pudo guardar la tarea: " + error.message); return; }
    setTasks((prev) => [mapTask(data), ...prev]);
    logActivity(meName, `creó la tarea ${title}`, "tareas");
  };

  const addBox = async ({ label, room, fragile }) => {
    const { data, error } = await supabase.from("boxes").insert({ household_id: householdId, label, room, fragile, status: "Vacía", contents: [], by_name: meName }).select().single();
    if (error) { console.error("Error creando caja:", error); alert("No se pudo guardar la caja: " + error.message); return; }
    setBoxes((prev) => [mapBox(data), ...prev]);
    logActivity(meName, `agregó ${label}`, "cajas");
  };

  const addSale = async ({ item, room, estimated }) => {
    const { data, error } = await supabase.from("sales").insert({ household_id: householdId, item, room, estimated, listed: null, status: "Pendiente", by_name: meName }).select().single();
    if (error) { console.error("Error registrando venta:", error); alert("No se pudo registrar la venta: " + error.message); return; }
  ...
  const addMember = async (name) => {
    ...
    const { data, error } = await supabase.from("household_members").insert({ household_id: householdId, name, role: "Miembro", color_key }).select().single();
    if (error) { console.error("Error agregando persona:", error); alert("No se pudo agregar a " + name + ": " + error.message); return; }
  ...
  return (
    <div className="relative w-full min-h-screen" style={{ background: C.bg, fontFamily: FONT }}>
      ...
      <SidebarMenu open={menuOpen} onClose={() => setMenuOpen(false)} tab={tab} setTab={(t) => { setTab(t); setMoreScreen(null); }} />
