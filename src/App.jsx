...
const photoUrl = (key, w = 400, h = 400) => {
  ...
  return `https://source.unsplash.com/${w}x${h}/?${imgSeed[key] || key || "furniture"}`;
};
...
function TopBar({ onOpenMenu, title = "Mi mudanza" }) {
  return (
    <div
    ...
    <div className="fixed inset-0 z-40">
      ...
      >
        <div className="flex items-center justify-between px-5 mb-6">
          <span className="text-base font-extrabold" style={{ color: C.ink }}>Mi mudanza</span>
          <button onClick={onClose} className="p-1.5 rounded-full" style={{ background: C.line }}><X size={16} /></button>
        </div>
...
export default function App() {
  ...
  const addItem = async ({ name, room, photoFile }) => {
    ...
    const { error } = await supabase.from("items").insert({ household_id: householdId, name, room, decision: "Sin decidir", img, by_name: meName });
  ...
  const addTask = async ({ title, category, due, priority }) => {
    const { error } = await supabase.from("tasks").insert({ household_id: householdId, title, category, due, priority, done: false, by_name: meName });
  ...
  const addBox = async ({ label, room, fragile }) => {
    const { error } = await supabase.from("boxes").insert({ household_id: householdId, label, room, fragile, status: "Vacía", contents: [], by_name: meName });
  ...
  const addSale = async ({ item, room, estimated }) => {
    const { error } = await supabase.from("sales").insert({ household_id: householdId, item, room, estimated, listed: null, status: "Pendiente", by_name: meName });
  ...
  const addMember = async (name) => {
    ...
    const { error } = await supabase.from("household_members").insert({ household_id: householdId, name, role: "Miembro", color_key });
